...
# Updating apk index
RUN yum update -y && yum install -y bash curl python build-dependencies ca-certificates wget which useradd
...
# RUN wget https://raw.githubusercontent.com/cityzendata/warp10-platform/master/etc/conf-distributed.template -O /opt/warp10-${WARP10_VERSION}/etc/warp.conf
